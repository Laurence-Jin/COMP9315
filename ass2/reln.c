...
struct RelnRep {
	...
	Count  depth;  // depth of main data file = d
...
void splitRelation(Reln r) {
	...
	while (pid != NO_PAGE) {
		...
		if (!pageNTuples(page)) break; 

		//get next tuple used by FILE
		char tuple[MAXTUPLEN];
		Offset postionBase = PAGESIZE * pid + 2 * sizeof(Offset) + sizeof(Count);
		fseek(f, postionBase + currTupleOffset, SEEK_SET);
		fgets(tuple, MAXTUPLEN - 1, f);
		Tuple curTuple = copyString(tuple);

		Bits p = getLower(tupleHash(r, curTuple), r->depth + 1); 

		addToRelationPage(r, p, curTuple);

		currTupleOffset += strlen(curTuple) + 1; 
		free(curTuple);
		nTuples++;

		if (nTuples < pageNTuples(page)) {
			continue;
		}
		else {
			// indicate overflow and add a new page
